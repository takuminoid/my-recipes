...
export async function POST(request: NextRequest) {
  ...
  const { name, steps, cookedAt, tint, photo, refUrl, rating, memo, ingredients } = body;
  ...
  const result = db.transaction(() => {
    const inserted = db
      .insert(recipes)
      .values({ name, steps, cookedAt, tint, photo, refUrl, rating, memo, createdAt: now, updatedAt: now })
      .returning()
      .all();
